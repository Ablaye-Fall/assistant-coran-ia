...
def qa_multilang(user_question):
    ...
    # Nettoyage
    cleaned = nettoyer_html(combined)
...
choix_surah = st.selectbox("📚 Choisissez une sourate :", surah_names, key="choix_surah")
...
traduction_choisie = st.selectbox("🌐 Choisissez une traduction :", list(translations.keys()), key="choix_traduction")
...
verset_num = st.number_input("📌 Choisissez un verset :", min_value=1, max_value=max_verset, value=1, key="choix_verset")
...
zoom = st.slider("🔍 Zoom texte arabe", min_value=1.0, max_value=3.0, value=1.5, step=0.1, key="zoom_arabe")
...
reciter_choice = st.selectbox("🎧 Choisissez un récitateur :", list(reciters.keys()), key="choix_reciteur")
...
lang_tafsir = st.selectbox("🌐 Langue traduction du tafsir :", ["fr", "en", "ar", "es", "wo"], key="choix_lang_tafsir")
...
# Q&A multilingue
@st.cache_resource
def resumer_texte_cached(texte, langue="fr"):
    """
    Résume le texte fourni en 2-3 phrases.
    Mis en cache pour éviter de recalculer inutilement.
    """
    try:
        phrases = texte.split(". ")
        resume = ". ".join(phrases[:3])
        return resume.strip()
    except Exception:
        return texte
...
for i, chat in enumerate(st.session_state.history):
    ...
    # 🔹 Ajout du résumé juste après la réponse
    resume = resumer_texte_cached(chat['answer'])
    try:
        resume_translated = GoogleTranslator(source="auto", target=detect(chat['question'])).translate(resume)
        st.markdown(f"**Résumé :** {resume_translated}")
    except:
        st.markdown(f"**Résumé :** {resume}")

user_q = st.text_input("💬 Posez votre question :", key="input_question")
...
if st.button("🗑 Effacer la conversation", key="btn_effacer"):
    st.session_state.history = []
